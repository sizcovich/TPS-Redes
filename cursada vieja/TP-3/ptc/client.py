...
class PTCClientProtocol(object):
	...
	def __init__(self, address, port):
		...
		self.init_transfer_time = None
		self.end_transfer_time = None
		self.retransmition_count = 0
	...
	def send_file(self, data):
		#linestring = open("1kb.txt", 'r').read()
		#inicializar reloj aca
		self.init_transfer_time = time.time()
		self.send(data.get_data())
		#time.sleep(2)
		print "Tiempo de transferencia para {0} Kb...".format(data.size() / 1000)
	...
	def handle_incoming(self, packet):
		...
		# * Distinguir el caso donde el estado es SYN_SENT
			...
			#   * No olvidar de hacer self.connected_event.set() al confirmar el ACK y establecer la conexión!!!
					...
					print "{0}".format(self.control_block.get_window_lo())
				...
				if self.control_block.accept_ack(packet.get_ack_number()):
					...
					if self.outgoing_buffer.empty() and self.retransmission_queue.empty():
						self.end_transfer_time = time.time() - self.init_transfer_time
						print "retransmisiones {0} ".format(self.retransmition_count)
						print "Tiempo {0} ms".format(self.end_transfer_time * 1000)
				else: 
