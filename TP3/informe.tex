...
Para la comparación del RTT contra el RTO se utilizó la norma 2 de vectores:
...
\section{Análisis}
En los gráficos de la sección anterior pudimos observar que los mejores valores que aproximan el RTO al RTT son utilizando $ 0.8 \leq \alpha \leq 1$ y $ 0.3 \leq  \beta \leq 0.6$. Estos valores fueron los obtenidos en los test realizados agregando delay para simular congestión en la red.
Sin embargo, en los gráficos realizados para medir la cantidad de retransmisiones, vemos que los mejores valores obtenidos para que haya la mínima cantidad de las mismas es de $ 0.1 \leq \alpha \leq 0.3$ y $ 0.1 \leq  \beta \leq 0.3$.


\section{Conclusiones}
De la experimentación pudimos ver como obtener valores de $\alpha$ y $\beta$ que mejor aproximen al RTT o minimicen la cantidad de retransmisiones de los paquetes.
Con la experimentación realizada y los valores obtenidos de $\alpha$ y $\beta$ cercanos a los recomendados en el RFC 6298 concluimos que los mismos seguramente fueron elegidos teniendo en cuenta las variables analizadas en el transcurso del trabajo práctico.
