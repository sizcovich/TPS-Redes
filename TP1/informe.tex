...
ARP (Address Resolution Protocol) consiste en un protocolo usado frecuentemente por las redes locales para conectar las capas 3 (capa de red) y 2 (capa de enlace) mediante la conversión o identificación de direcciones IP con direcciones Ethernet (direcciones MAC).
...
En este caso, el router que actúa como $gateway$ es la IP 192.168.1.1. 
...
\hline
CECEN-Wifi & $S_{src}$ & 1.56 \\
CECEN-Wifi & $S_{dst}$ & 5.17 \\
\hline
Starbucks   & $S_{src}$ & 1.84 \\
Starbucks   & $S_{dst}$ & 7.44 \\
\hline
Empresa    & $S_{src}$ & 3.67 \\
Empresa    & $S_{dst}$ & 3.12 \\
\hline
...
Por otro lado, notamos que, en la práctica, el modelo teórico de información/entropía no ayuda a distinguir nodos significativos pero que el modelo topológico resultó más útil para ello. Esto se debe a que en el modelo topológico no se tuvieron en cuenta los paquetes repetidos.
\section{Referencias}
\begin{itemize}
